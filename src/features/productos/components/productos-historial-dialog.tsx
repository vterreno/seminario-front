            ...
            <DialogContent className="max-w-7xl max-h-[90vh] overflow-hidden flex flex-col">
                <DialogHeader className="flex-shrink-0">
                    <DialogTitle className="text-lg sm:text-xl">
                        <span className="hidden sm:inline">Historial de movimientos: {currentRow.nombre}</span>
                        <span className="sm:hidden">Historial: {currentRow.nombre}</span>
                    </DialogTitle>
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 text-sm text-muted-foreground">
                        <span>Código: <span className="font-medium">{currentRow.codigo}</span></span>
                        <span>Stock actual: <strong className="text-foreground">{currentRow.stock}</strong></span>
                    </div>
                </DialogHeader>

                
                <div className="flex-1 overflow-y-auto flex flex-col space-y-4 max-h-[calc(90vh-200px)]">
                    {/* Estadísticas resumidas */}
                    {!loading && movimientos.length > 0 && (
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 flex-shrink-0">
                            <Card className="p-3">
                                <CardContent className="p-0">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-lg font-semibold truncate">{estadisticas.totalMovimientos}</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="p-0">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-lg font-semibold text-green-600 truncate">{estadisticas.aumentos}</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="p-0">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-lg font-semibold text-red-600 truncate">{estadisticas.disminuciones}</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                            <Card className="p-3">
                                <CardContent className="p-0">
                                    <div className="min-w-0">
                                        ...
                                        <p className="text-lg font-semibold text-green-600 truncate">+{estadisticas.totalAumentado}</p>
                                    </div>
                                </CardContent>
                            </Card>
                            <Card className="p-3">
                                <CardContent className="p-0">
                                    <div className="min-w-0">
                                        ...
                                        <p className="text-lg font-semibold text-red-600 truncate">-{estadisticas.totalDisminuido}</p>
                                    </div>
                                </CardContent>
                            </Card>
                        </div>
                    ...
                    <div className="flex-1 overflow-hidden flex flex-col">
                        {loading ? (
                            ...
                            <>
                                            ...
                                            <TableRow>
                                                <TableHead className="w-[120px] sm:w-[140px]">
                                                    ...
                                                    >
                                                        ...
                                                        <ArrowUpDown className="ml-1 sm:ml-2 h-3 w-3" />
                                                    </Button>
                                                </TableHead>
                                                ...
                                                <TableHead className="min-w-[150px] sm:min-w-[200px]">
                                                    <span className="text-xs sm:text-sm font-semibold">Descripción</span>
                                                </TableHead>
                                                <TableHead className="w-[80px] sm:w-[100px] text-right">
                                                    <Button
                                                        ...
                                                        className="h-auto p-0 font-semibold hover:bg-transparent ml-auto text-xs sm:text-sm"
                                                    >
                                                        Cantidad
                                                        <ArrowUpDown className="ml-1 sm:ml-2 h-3 w-3" />
                                                    </Button>
                                                ...
                                                <TableHead className="w-[100px] sm:w-[120px] text-right">
                                                    <span className="text-xs sm:text-sm font-semibold">
                                                        <span className="hidden sm:inline">Stock resultante</span>
                                            ...
                                            {paginatedMovimientos.map((movimiento) => (
                                                <TableRow key={movimiento.id}>
                                                        ...
                                                        <div className="flex flex-col">
                                                            <span>
                                                                {movimiento.fecha || movimiento.created_at
                                                                    ? format(new Date(movimiento.fecha ?? movimiento.created_at!), 'dd/MM/yyyy', { locale: es })
                                                                    : 'Sin fecha'}
                                                            </span>
                                                            <span className="text-xs text-muted-foreground">
                                                                {movimiento.fecha || movimiento.created_at
                                                                    ? format(new Date(movimiento.fecha ?? movimiento.created_at!), 'HH:mm', { locale: es })
                                                                    : ''}
                                                            </span>
                                                        </div>
                                                    </TableCell>
                                                    <TableCell>
                                                        ...
                                                        >
                                                            <span className="hidden sm:inline">{formatTipoMovimiento(movimiento.tipo_movimiento)}</span>
                                                            <span className="sm:hidden">
                                                                {movimiento.tipo_movimiento === 'STOCK_APERTURA' && 'Apertura'}
                                                                {movimiento.tipo_movimiento === 'VENTA' && 'Venta'}
                                                                {movimiento.tipo_movimiento === 'COMPRA' && 'Compra'}
                                                                {movimiento.tipo_movimiento === 'AJUSTE_MANUAL' && 'Ajuste'}
                                                            </span>
                                                        </Badge>
                                                    </TableCell>
                                                    <TableCell className="max-w-[200px] sm:max-w-[250px]">
                                                        <div className="truncate text-xs sm:text-sm" title={movimiento.descripcion}>
                                                            {movimiento.descripcion}
                                                        </div>
                                                    </TableCell>
                                                    <TableCell className="text-right font-medium">
                                                        <span className={`text-xs sm:text-sm ${movimiento.cantidad >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                ...
                                {/* Paginación */}
                                {totalPages > 1 && (
                                    <div className="flex flex-col sm:flex-row items-center justify-between pt-4 border-t flex-shrink-0 gap-2">
                                        <div className="text-xs sm:text-sm text-muted-foreground text-center sm:text-left">
                                            <span className="hidden sm:inline">Página {currentPage} de {totalPages} ({filteredAndSortedMovimientos.length} movimientos)</span>
                                            <span className="sm:hidden">{currentPage}/{totalPages} ({filteredAndSortedMovimientos.length})</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <Button
                                                variant="outline"
